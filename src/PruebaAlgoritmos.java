...
class AlgoritmoBellmanFord {
    ...
    public AlgoritmoBellmanFord(int numberofvertices){
        ...
        for (int sourcenode = 1; sourcenode <= numberofvertices; sourcenode++){
            for (int destinationnode = 1; destinationnode <= numberofvertices; destinationnode++){
                if (adjacencymatrix[sourcenode][destinationnode] != MAX_VALUE){
                    if (distances[destinationnode] > distances[sourcenode]
                           + adjacencymatrix[sourcenode][destinationnode])
                        System.out.println("El gráfico contiene ciclo egde negativo");
                }
            }
        }
        for (int vertex = 1; vertex <= numberofvertices; vertex++){
            System.out.println("Distancia de vertices " + source + " a "
                      + vertex + " es " + distances[vertex]);
        }
    }
}
	...
	public static void main(String[] args) {
		...
		do {
			...
			case 3:
		        ...
		        System.out.println("Ingrese los valores a la matriz de adyacencia: ");
		        System.out.println("\n Ejemplo:" + " \n 0 4 0 0 -1 0 \n" + " X X X X  X X \n");
		        for (int sourcenode = 1; sourcenode <= numberofvertices; sourcenode++){
		            for (int destinationnode = 1; destinationnode <= numberofvertices; destinationnode++){
		        ...
		        System.out.println("Ingrese el vértice inicial: ");
		        source = scanner.nextInt();
		        AlgoritmoBellmanFord bellmanford = new AlgoritmoBellmanFord(numberofvertices);
